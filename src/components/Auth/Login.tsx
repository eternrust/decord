...
import { postLogin, PostLoginRequest } from "@/api/auth/postLogin";
...
const Login = ({ onChangeModal, onClose }: Props) => {
    const [data, setData] = useState<PostLoginRequest>({
        id: '',
        pw: ''
    })
    ...
    const submitLogin = useCallback(async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        await postLogin(data)
            .then(res => {
                toast.success("로그인에 성공하셨습니다!", { id: toastId })
    ...
    return (
        <div className="flex flex-col py-10 gap-5 w-[460px] items-center">
            <div>
                ...
                <span className="text-labelLarge_bold text-blue600 mx-auto mt-2 block w-fit">로그인</span>
            </div>
            <form className="w-[350px] flex flex-col gap-8">
                ...
                />
                <Button type="submit" isFull onClick={submitLogin}>로그인</Button>
            </form>
            <div className="inline-block text-labelSmall_semiBold">
                <span className="text-gray800">아직 회원이 아니신가요?</span>
                <span className="mx-5 text-blue400 hover:text-blue700 cursor-pointer" onClick={() => onChangeModal()}>회원가입</span>
            </div>
        </div>
    )
}
